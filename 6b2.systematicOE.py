...
file_path = "./Systematic_OE_results_all.celloracle.hdf5" # Please use .hdf5 for extension.


def pipeline(gene_for_OE):
     
    # 1. Simulate OE
    oe=1.5*float(max(oracle.adata[:,gene_for_OE].layers["imputed_count"]))
    oracle.simulate_shift(perturb_condition={gene_for_OE: oe},
                                 ignore_warning=True,
                                 n_propagation=3)
